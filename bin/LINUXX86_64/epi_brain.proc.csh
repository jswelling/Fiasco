...
# ************************************************************/
#
# $Id: epi_brain.proc.csh,v 1.8 2007/06/21 23:13:20 welling Exp $
echo '#'`date` $0

echo "The use of epi_brain is deprecated.  Use epi with "
echo "F_STATS_SCRIPT set to calc_stats_brain.csh ."
exit -1

if(! -e fiasco.local.csh) cp $FIASCO/fiasco.local.csh .
source fiasco.local.csh
if (-e epi_brain.defaults.csh) then
  source epi_brain.defaults.csh
else
  source $FIASCO/epi_brain.defaults.csh
endif
if(! -e epi_brain.local.csh) cp $FIASCO/epi_brain.local.csh .
source epi_brain.local.csh 
...
set rawScriptName = epi_brain.steps.csh
