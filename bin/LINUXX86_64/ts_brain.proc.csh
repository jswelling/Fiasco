...
# ************************************************************/
#
# $Id: ts_brain.proc.csh,v 1.4 2007/06/21 23:13:20 welling Exp $
echo '#'`date` $0

echo "The use of ts_brain is deprecated.  Use ts with "
echo "F_STATS_SCRIPT set to calc_stats_brain.csh ."
exit -1

if(! -e fiasco.local.csh) cp $FIASCO/fiasco.local.csh .
source fiasco.local.csh
if (-e ts_brain.defaults.csh) then
  source ts_brain.defaults.csh
else 
  source $FIASCO/ts_brain.defaults.csh
endif
if(! -e ts_brain.local.csh) cp $FIASCO/ts_brain.local.csh .
source ts_brain.local.csh 
...
set rawScriptName = ts_brain.steps.csh
