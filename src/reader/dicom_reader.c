...
static void processHeader( FileHandler* self, KVHash* info, SList* cStack )
{
  ...
  /* This seems to be useful when decoding some CT images */
  if ((!kvLookup(info,"dx") || (kvGetInt(info,"dx")<=0))
      && kvLookup(info,"dx_mosaic") && (kvGetInt(info,"dx_mosaic")>1))
    kvDefInt(info,"dx",kvGetInt(info,"dx_mosaic"));
  if ((!kvLookup(info,"dy") || (kvGetInt(info,"dy")<=0))
      && kvLookup(info,"dy_mosaic") && (kvGetInt(info,"dy_mosaic")>1)) 
    kvDefInt(info,"dy",kvGetInt(info,"dy_mosaic"));
