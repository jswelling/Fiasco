...
#
echo '#'`date` $0
echo '#$Id: spiral.printpar.csh,v 1.22 2006/04/07 17:54:20 welling Exp $'
...
foreach fname ( $tmp1 $tmp2 $tmp3 $tmp4 $tmp5 $tmp6 $tmp7 $tmp8)
  if ( -e $fname ) rm $fname
end
...
set despikeparms = `most_recent_parms.py despike`
if ( dummy${despikeparms} != dummy ) then
  egrep -v '#' $despikeparms > $tmp6
endif

set outlierparms = `most_recent_parms.py outlier`
if ( dummy${outlierparms} != dummy ) then
  egrep -v '#' $outlierparms > $tmp7
endif

cat $FIASCO/../../splus/script/spiralprintpar.S | \
  sed "s/nslice/$F_NSLICE/g" | \
  sed "s/name/$usename/g" | sed "s/inc/$F_PRINTPAR_MAXPLOT/g" | \
  sed "s/nimage/$F_NIMAGE/g" | sed "s*tmpdir*$F_TEMP*g" | \
  sed "s/nslice/$F_NSLICE/g" | sed "s*meancprm*$tmp5*g" | \
  sed "s*spikeprm*$tmp6*g" | sed "s*outlierprm*$tmp7*g" | \
  $SPLUS>>& out/S.output
