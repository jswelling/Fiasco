...
# ************************************************************/
#
# $Id: epi.proc.csh,v 1.7 2007/06/21 23:13:20 welling Exp $
echo '#'`date` $0
if(! -e fiasco.local.csh) cp $FIASCO/fiasco.local.csh .
source fiasco.local.csh
if (-e epi.defaults.csh) then
  source epi.defaults.csh
else
  source $FIASCO/epi.defaults.csh
endif
if(! -e epi.local.csh) cp $FIASCO/epi.local.csh .
source epi.local.csh 
...
set rawScriptName = epi.steps.csh
