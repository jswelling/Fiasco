 ...
 *		9/99: Made all byte offsets 64 bits long to
 *			accommodate datasets larger than 2GB
 */
...
#define MRI_INT64	5	/* 64 bit integer */
...
#define MRI_COMPLEX_LONG	(MRI_COMPLEX | MRI_LONG)
#define MRI_COMPLEX_LONGLONG	(MRI_COMPLEX | MRI_LONGLONG)
...
#define MRI_VECTOR_LONG		(MRI_VECTOR | MRI_LONG)
#define MRI_VECTOR_LONGLONG	(MRI_VECTOR | MRI_LONGLONG)
...
typedef struct MRI_Buffer {
  ...
  long long size;		/* the size of the buffer in bytes */
...
typedef struct MRI_Dataset {
  ...
  /* header info */
  long long header_size;	/* the number of bytes reserved for
				   the header */
...
#ifdef AFS
  /* Special support for AFS file system */
  int some_parts_in_afs;        /* non-zero if header or a chunk is on afs */
#endif
} MRI_Dataset;
...
typedef struct MRI_Chunk {
  ...
  long long extent[MRI_MAX_DIMS]; /* the size of each dimension */
  int little_endian;		/* if TRUE, the chunk is stored in
				   little-endian format */
  MRI_Order order;		/* specifies where the chunk
				   should go in its file */
  long long offset;		/* specifies the absolute byte offset
				   where the chunk should go in its file */
  long long size;		/* the size of the chunk in bytes */
  ...
  long long actual_extent[MRI_MAX_DIMS];
 ...
 *      ACCESSOR MACROS
 *-----------------------------------------------------------------------*/
#define MRI_DATASET_NAME( ds ) (ds->name)
...
/*-------- GETTING KEY VALUES ------------------------------------*/
extern long long mri_get_int (MRI_Dataset *ds, const char *key);
extern double mri_get_float (MRI_Dataset *ds, const char *key);
extern char *mri_get_string (MRI_Dataset *ds, const char *key);

/*-------- SETTING KEY VALUES ------------------------------------*/
extern void mri_set_int (MRI_Dataset *ds, const char *key, const long long value);
...
extern void mri_set_chunk (MRI_Dataset *ds, const char *key,
			   long long size, long long offset,
			   MRI_ArrayType type, void *ptr);
extern void *mri_read_chunk (MRI_Dataset *ds, const char *key,
			     long long size, long long offset,
			     MRI_ArrayType type, void* buffer);
#define mri_write_chunk mri_set_chunk
