 ...
 *     int dim, void* info, float *(*access_coords)(float *, int, void* *) )
 *
 * void dch_destroy_tesselation ( dch_Tess *tess_to_destroy )
 *
 ...
 *
 * float *access_coords( void *coorddata, int ipt, void* info, void* *hook )
 *
 ...
 * void* which the access_coords routine may set;  the value
 ...
 *
 * If the point to be added is *too* close to the closest existing point
 * (as determined by the PROXIMITY_TOLERANCE and the function too_close()),
 * the new point becomes a "twin" of the exiting point and is not processed 
 * further.  It will not form a point in the Delaunay triangulation and
 * will not have an associated Voronoi polygon, but will be found in the
 * "twins" list of a point which is included in the tesselation.
 *
...
static char rcsid[] = "$Id: dirichlet.c,v 1.4 2005/07/07 20:02:27 welling Exp $";
...
#define PROXIMITY_TOLERANCE 0.000000001 /* factor used to determine if points
...
/* Back substitution */
  ...
  if ( !(ans = ( float * ) malloc( n*sizeof( float ) ) ) )
    Abort("dirichlet: gauss_elim: unable to allocate %d floats!\n",n);
...
/***************************************
 * End of Gaussian elimination routine
 ***************************************/
...
static void mem_init(int npts, int dim)
{
  ...
  if (gauss_matrix) free( (void*)gauss_matrix );
  if ( !(gauss_matrix= (float *)malloc( dim*(dim+1)*sizeof(float) )) )
    Abort("dirichlet: mem_init: unable to allocate %d floats!\n",dim*(dim+1));
}
 ...
 */
{
  ...
  if(!coords1 || !coords2) 
    Abort("dirichlet: distance: Null coords in distance!\n");
...
static dch_Vtx *create_vtx( float *coords )
{
  ...
  if (!free_vtx_cells) {
    if ( !(vtx= (dch_Vtx *)malloc(sizeof(dch_Vtx))) )
      Abort("dirichlet: create_vtx: unable to allocate %d bytes!\n",
	    sizeof(dch_Vtx));
  }
  else {
    vtx= free_vtx_cells;
  ...
  if (coords) { /* don't do this for vertices at infinity */
    if ( !(vtx->coords= (float *)malloc(current_dim * sizeof(float))) )
      Abort("dirichlet:create_vtx: unable to allocate %d floats!\n",
	    current_dim);
...
/* Add the given vertex to the given list */
{
  ...
  if ( !free_vtx_list_cells ) { /* need to create a cell */
    if ( !(temp= (dch_Vtx_list *)malloc(sizeof(dch_Vtx_list))) )
      Abort("dirichlet: add_list_vtx: unable to allocate %d bytes!\n",
		sizeof(dch_Vtx_list));
  }
  else {
			  ...
			  dch_Vtx *vtx1, dch_Vtx *vtx2 )
{
  ...
  if ( !(temp= (dch_Vtx_pair_list *)malloc(sizeof(dch_Vtx_pair_list))) )
    Abort("dirichlet: add_pair_vtxs: unable to allocate %d bytes!\n",
...
static void destroy_vtx( dch_Vtx *vtx )
{
  ...
  if (vtx->coords) free( (void*)vtx->coords );
...
static dch_Pt *create_pt( float *coords )
{
  ...
  if ( !(pt= (dch_Pt *)malloc(sizeof(dch_Pt))) )
    Abort("dirichlet: create_pt: unable to allocate %d bytes!\n",
	  sizeof(dch_Vtx));
  if ( !(pt->coords= (float *)malloc(current_dim * sizeof(float))) )
    Abort("dirichlet: create_pt: unable to allocate %d floats!\n",current_dim);

  for (i=0; i<current_dim; i++) pt->coords[i]= coords[i];

  pt->neighbors= (dch_Pt_list *)0;
  pt->twins= (dch_Pt_list *)0;
...
{
  if (!pt) Abort("dirichlet: Tried to free a null point; algorithm error!\n");

  destroy_pt_list( pt->neighbors );
  destroy_pt_list( pt->twins );
...
/* Add the given point to the given list */
{
  ...
  if (!free_pt_list_cells) { /* need to allocate a new cell */
    if ( !(temp= (dch_Pt_list *)malloc(sizeof(dch_Pt_list))) )
      Abort("dirichlet: add_list_pt: unable to allocate %d bytes!\n",
		sizeof(dch_Pt_list));
  }
  else {
...
static void add_pair_pts( dch_Pt_pair_list **pplist, dch_Pt *pt1, dch_Pt *pt2 )
{
  ...
  if ( !(temp= (dch_Pt_pair_list *)malloc(sizeof(dch_Pt_pair_list))) )
    Abort("dirichlet: add_pair_pts: unable to allocate %d bytes!\n",
...
/* This routine is for debugging only */
{
  ...
  while (plist) {
    ...
    else Abort("dirichlet: Tried to dump a null point; algorithm error!\n");
    plist= plist->next;
  }
  fprintf(stderr," )\n");
}
...
static dch_Bndbx *create_bndbx()
{
  ...
  if ( !(result= (dch_Bndbx *)malloc(sizeof(dch_Bndbx))) )
    Abort("dirichlet: create_bndbx: unable to allocate %d bytes!\n",
	  sizeof(dch_Bndbx));
  if ( !(result->corner1= (float *)malloc(current_dim*sizeof(float))) )
    Abort("dirichlet: create_bndbx: unable to allocate %d floats!\n",
	  current_dim);
...
{
  free( (void*)bndbx->corner1 );
  free( (void*)bndbx->corner2 );
  free( (void*)bndbx );
}
 ...
 */
{
  ...
  if ( !(coords= (float *)malloc(current_dim*sizeof(float))) )
    Abort("dirichlet: calc_bogus_pts: unable to allocate %d floats!\n",
	  current_dim);
 ...
 */
{
  ...
  if ( !(point_array= (dch_Pt **)malloc( (current_dim+1)*sizeof(dch_Pt *) )) )
    Abort("dirichlet: add_first_vtx_neighbors: unable to allocate %d bytes!\n",
...
static dch_Tess *create_initial_tesselation( dch_Bndbx *bndbx )
{
  ...
  if ( !(tess= (dch_Tess *)malloc(sizeof(dch_Tess))) )
    Abort("dirichlet: create_initial_tesselation: unable to allocate %d bytes!\n",
              sizeof(dch_Tess));
...
#ifdef never
  fprintf(stderr,
	  "Final counts: created_vtx_count %d, created_vtx_highwater %d\n",
	  created_vtx_count,
	  created_vtx_highwater);
  fprintf(stderr,
	  "              created_pt_count %d, created_pt_highwater %d\n",
	  created_pt_count,
	  created_pt_highwater);
  fprintf(stderr,
	  "              created_vtx_list_count %d, created_vtx_list_highwater %d\n",
	  created_vtx_list_count,
	  created_vtx_list_highwater);
  fprintf(stderr,
	  "              created_pt_list_count %d, created_pt_list_highwater %d\n",
	  created_pt_list_count,
	  created_pt_list_highwater);
  fprintf(stderr,
	  "              created_vtx_pair_count %d, created_vtx_pair_highwater %d\n",
	  created_vtx_pair_count,
	  created_vtx_pair_highwater);
  fprintf(stderr,
	  "              created_pt_pair_count %d, created_pt_pair_highwater %d\n",
	  created_pt_pair_count,
...
static dch_Vtx *find_vtx_to_delete( dch_Tess *tess, dch_Pt *pt)
{
  ...
  case (int)CENTER_PT: 
    closest_pt= find_closest_pt( tess->center_pt, pt, &(tess->center_steps));
    break;
  default: Abort("dirichlet: find_vtx_to_delete: unknown method %d!\n");
  }
  tess->searches_done++;
   ...
   * The new point becomes a 'twin' of the closest point.
   */
  if ( too_close( closest_pt->coords, pt->coords, tess ) ) {
    add_list_pt( &(closest_pt->twins), pt );
    return( (dch_Vtx *)0 );
  }
  ...
  Abort("dirichlet: find_vtx_to_delete: no deleted vertex found!\n");
 ...
 */
{
  ...
  while (plist) {
	...
	if ( vl->vtx->degenerate )
	  fprintf(stderr,"accumulate_vtxs: found degenerate vertex %d",
		    vl->vtx->id);
	else if ( !(vl->vtx->coords) )
	  fprintf(stderr,"accumulate_vtxs: found infinite vertex %d", 
		  vl->vtx->id);
#endif
      }
      vl= vl->next;
 ...
 */
{
  ...
  if ( !pt_in_bndbx( tess->bndbx, pt ) ) {
    Error("dirichlet: add_pt_to_tesselation: point outside bounding box; ignored.\n");
    return;
  }
   ...
   * The solution is to drop the new point, which has become a "twin"
   * of the existing point.
   */
  if (!deleted_vtx) {
    Error(
      "dirichlet: add_pt_to_tesselation: Point %d at (%f %f ...) is a twin of an existing point.\n",
      pt->id,pt->coords[0],pt->coords[1]);
    return;
  }
...
static dch_Pt *create_user_pt( void *coorddata, int i, void* info,
...
/* This function just invokes the user's coordinate access function */
{
  ...
  result= create_pt( (*coord_access_fun)( coorddata, i, info, &uhook ) );
  result->user_hook= uhook;
  return( result );
}

dch_Tess *dch_create_dirichlet_tess( void *coorddata, 
				     int npts, int dimensionality, void* info,
				     float *(*coord_access_fun)(void *, 
								int, void*,
								void**) )
{
  int i;
  ...
  if (npts<1) {
    Error("dirichlet: create_dirichlet_tesselation: %d is not enough input points!\n",
	      npts);
    return( (dch_Tess *)0 );
  }
  ...
  for (i=0; i<npts; i++)
    add_list_pt( &input_pt_list, 
		create_user_pt( coorddata, i, info, coord_access_fun ) );
