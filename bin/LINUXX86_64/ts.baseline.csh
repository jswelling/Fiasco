...
# ************************************************************/
#
echo '$Id: ts.baseline.csh,v 1.16 2003/05/14 18:15:54 welling Exp $'
#
echo '#'`date`$0
if(! -d par) mkdir par
...
if ( $dimorder != 'vxyzt' ) then
  echo "ts.baseline.csh: input is not in vxyzt order!"
  exit -1
endif
set vdim = `mri_printfield -i $1 -field images.extent.v`
if ( $vdim != 2) then
  echo "ts.baseline.csh: input is not complex!"
  exit -1
endif
@ tdim_half = ( $tdim / 2 )
@ zdim_dbl = ( $zdim * 2 )
